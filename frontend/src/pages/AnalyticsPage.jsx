...
export default function AnalyticsPage() {
  ...
  async function load() {
    ...
    sales.forEach(s => {
      ...
      dayMap[d] = dayMap[d] || { date: d, Satış: 0, Gider: 0 };
      dayMap[d].Satış += s.total;
    });
    expenses.forEach(e => {
      const d = e.date.slice(0, 10);
      dayMap[d] = dayMap[d] || { date: d, Satış: 0, Gider: 0 };
      dayMap[d].Gider += e.amount;
    });
    ...
    const locMap = {};
    sales.forEach(s => {
      const name = locs.find(l => l.id === s.locationId)?.name ?? 'Diğer';
      locMap[name] = locMap[name] || { name, Satış: 0, Gider: 0 };
      locMap[name].Satış += s.total;
    });
    expenses.forEach(e => {
      const name = locs.find(l => l.id === e.locationId)?.name ?? 'Diğer';
      locMap[name] = locMap[name] || { name, Satış: 0, Gider: 0 };
      locMap[name].Gider += e.amount;
    });
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 pb-10">
      <h1 className="text-2xl font-bold mb-5 text-gray-800">Analiz</h1>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <StatCard label="Toplam Satış"  value={stats.sales}    color="text-emerald-600" />
        <StatCard label="Toplam Gider"  value={stats.expenses} color="text-red-500" />
        <StatCard
          label="Net Kâr"
          value={stats.profit}
      ...
      {daily.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm mb-4">
          <h2 className="font-bold text-gray-700 mb-3 text-base">Günlük Satış / Gider</h2>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={daily} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
              ...
              <Legend />
              <Bar dataKey="Satış" fill="#10b981" radius={[4,4,0,0]} />
      ...
      {byLocation.length > 0 && (
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <h2 className="font-bold text-gray-700 mb-3 text-base">Pazar Yerlerine Göre</h2>
      ...
      {daily.length === 0 && byLocation.length === 0 && (
        <div className="text-center text-gray-400 py-16">
          ...
          <p>Bu tarih aralığında veri yok</p>
        </div>
      )}
    </div>
  );
